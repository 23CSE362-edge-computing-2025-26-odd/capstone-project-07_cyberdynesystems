...
class Server(): The class that implements the server side of the workflow
    ...
    choose_c(): calls the threshold_calc of the client and appends the client objects to the C list

    send_parameters(): sends the global parameters to the chosen C clients for training and for inferencing to K - C clients

    recv_updates() -> [parameters]: the recvd updates are stored and sent for fedAvg
...
class Server():
    ...
    '''
    send_parameters: (requires refining)
